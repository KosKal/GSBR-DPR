function fqPredictionGSB(x::Array{Float64}, T::Int64, gibbsIter::Int64, burnIn::Int64, gEps1::Float64, gEps2::Float64, thLow::Float64,
  thUp::Float64, zLow::Float64, zUp::Float64, papr::Float64, pbpr::Float64, thin::Int64, samplerSeed::Int64)


	srand(samplerSeed)

  filename = "/Results/GSB"
  ...
  p = 0.5
	...
	for iter in 1:gibbsIter
      ...
      ## Sample future unobserved observations
      ##########################################################################
      ...
      if T .> 0
        ...
        if T .> 1
          for i in 1:1:(T-2)
            ...
            a4 = Lambdas[d[n+i+1]] * theta[5] ^ 2 + Lambdas[d[n+i+2]] * theta[6] ^ 2

            a3 = 2. * Lambdas[d[n+i+1]] * (theta[4] * theta[5] * x[n+i-1] + theta[2] * theta[5]) + 2. * Lambdas[d[n+i+2]] * (theta[4] * theta[6] * x[n+i+1] + theta[3] * theta[6])

            a2 = Lambdas[d[n+i+1]] * (theta[4]^2 * x[n+i-1]^2 + 2. * theta[5] * theta[6] * x[n+i-1]^2 + 2. * theta[2] * theta[4] * x[n+i-1] + 2. * theta[3] * theta[5] * x[n+i-1] -
                                2. * theta[5] * x[n+i+1] + 2. * theta[1] * theta[5] + theta[2]^2) +
               Lambdas[d[n+i+2]] * (theta[4]^2 * x[n+i+1]^2 + 2. * theta[5] * theta[6] * x[n+i+1]^2 + 2 * theta[2] * theta[6] * x[n+i+1] + 2 * theta[3] * theta[4] * x[n+i+1] -
                                2. * theta[6] * x[n+i+2] + 2. * theta[1] * theta[6] + theta[3]^2) +
                Lambdas[d[n+i]]

            a1 = 2. * (Lambdas[d[n+i+1]] * (theta[4] * theta[6] * x[n+i-1]^3 + (theta[2] * theta[6] + theta[3] * theta[4]) * x[n+i-1]^2 + (-theta[4] * x[n+i+1] + theta[1] * theta[4] + theta[2] * theta[3]) * x[n+i-1] -
                                      theta[2] * x[n+i+1] + theta[1] * theta[2]) +
                    Lambdas[d[n+i+2]] * (theta[4] * theta[5] * x[n+i+1]^3 + (theta[2] * theta[4] + theta[3] * theta[5]) * x[n+i+1]^2 -
                                    theta[4] * x[n+i+1] * x[n+i+2] + (theta[1] * theta[4] + theta[2] * theta[3]) * x[n+i+1] - theta[3] * x[n+i+2] + theta[1] * theta[3]) - 
                    Lambdas[d[n+i]] * (theta[4] * x[n+i-2] * x[n+i-1] + theta[5] * x[n+i-1]^2 + theta[6] * x[n+i-2]^2 + theta[2] * x[n+i-1] + theta[3] * x[n+i-2] + theta[1])  )

            aux = -2. * log.(rand()) + a1 * x[n+i] + a2 * x[n+i] ^ 2 + a3 * x[n+i] ^ 3 + a4 * x[n+i] ^ 4
          ...
          # 2nd last future unobserved observation
            a4 = Lambdas[d[n+T]] * theta[5] ^ 2 

            a3 = 2. * Lambdas[d[n+T]] * (theta[4] * theta[5] * x[n+T-2] + theta[2] * theta[5]) 

            a2 = Lambdas[d[n+T]] * (theta[4]^2 * x[n+T-2]^2 + 2. * theta[5] * theta[6] * x[n+T-2]^2 + 2. * theta[2] * theta[4] * x[n+T-2] + 2. * theta[3] * theta[5] * x[n+T-2] -
                                2. * theta[5] * x[n+T] + 2. * theta[1] * theta[5] + theta[2]^2) +
                Lambdas[d[n+T-1]]

            a1 = 2. * (Lambdas[d[n+T]] * (theta[4] * theta[6] * x[n+T-2]^3 + (theta[2] * theta[6] + theta[3] * theta[4]) * x[n+T-2]^2 + (-theta[4] * x[n+T] + theta[1] * theta[4] + theta[2] * theta[3]) * x[n+T-2] -
                                      theta[2] * x[n+T] + theta[1] * theta[2]) - 
                    Lambdas[d[n+T-1]] * (theta[4] * x[n+T-3] * x[n+T-2] + theta[5] * x[n+T-2]^2 + theta[6] * x[n+T-3]^2 + theta[2] * x[n+T-2] + theta[3] * x[n+T-3] + theta[1])  )

            aux = -2. * log.(rand()) + a1 * x[n+T-1] + a2 * x[n+T-1] ^ 2 + a3 * x[n+T-1] ^ 3 + a4 * x[n+T-1] ^ 4
        ...
        a1 = - 2. * Lambdas[d[T]] * (theta[4] * x[n+T-2] * x[n+T-1] + theta[5] * x[n+T-1]^2 + theta[6] * x[n+T-2]^2 + theta[2] * x[n+T-1] + theta[3] * x[n+T-2] + theta[1]) 
        ...
        # new_mu = henon(theta, x[n+T-1], x[n+T-2])
        # new_var = 1./Lambdas[d[n+T]]
